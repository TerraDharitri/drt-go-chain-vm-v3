...
type VMHostStub struct {
	...
	ExecuteDCDTTransferCalled   func(destination []byte, sender []byte, transfers []*vmcommon.DCDTTransfer, callType vm.CallType) (*vmcommon.VMOutput, uint64, error)
...
func (vhs *VMHostStub) ExecuteDCDTTransfer(destination []byte, sender []byte, transfers []*vmcommon.DCDTTransfer, callType vm.CallType) (*vmcommon.VMOutput, uint64, error) {
	if vhs.ExecuteDCDTTransferCalled != nil {
		return vhs.ExecuteDCDTTransferCalled(destination, sender, transfers, callType)
	}
	return nil, 0, nil
}
