...
import (
	...
	contextmock "github.com/TerraDharitri/drt-go-chain-vm-v3/mock/context"
	"github.com/TerraDharitri/drt-go-chain-vm-v3/mock/contracts"
...
func TestGasUsed_AsyncDCDTTransfer(t *testing.T) {
	var parentAccount *worldmock.Account
	initialDCDTTokenBalance := uint64(100)
	...
	test.BuildMockInstanceCallTest(t).
		WithContracts(
			test.CreateMockContract(test.ParentAddress).
				...
				WithMethods(contracts.ExecDCDTTransferInAsyncCall, contracts.EvilCallback),
			test.CreateMockContract(test.ChildAddress).
				WithBalance(testConfig.ChildBalance).
				WithConfig(testConfig).
		...
		WithInput(test.CreateTestContractCallInputBuilder().
			WithRecipientAddr(test.ParentAddress).
			WithGasProvided(testConfig.GasProvided).
			WithFunction("dcdtTransferInAsyncCall").
