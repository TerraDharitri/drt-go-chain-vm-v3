...
import (
	...
	"github.com/TerraDharitri/drt-go-chain-vm-v3/vmhost/contexts"
)
...
func (host *vmHost) ExecuteDCDTTransfer(destination []byte, sender []byte, transfers []*vmcommon.DCDTTransfer, callType vm.CallType) (*vmcommon.VMOutput, uint64, error) {
	if len(transfers) == 0 {
	...
	if len(transfers) == 1 {
		if transfers[0].DCDTTokenNonce > 0 {
			...
			dcdtTransferInput.Arguments = append(dcdtTransferInput.Arguments, transfers[0].DCDTTokenName, nonceAsBytes, transfers[0].DCDTValue.Bytes(), destination)
		} else {
		...
		dcdtTransferInput.RecipientAddr = dcdtTransferInput.CallerAddr
		dcdtTransferInput.Arguments = append(dcdtTransferInput.Arguments, destination, big.NewInt(int64(len(transfers))).Bytes())
