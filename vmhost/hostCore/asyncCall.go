...
func (host *vmHost) isDCDTTransferOnReturnDataWithNoAdditionalData(
	...
	return host.isDCDTTransferOnReturnDataFromFunctionAndArgs(sndAddr, dstAddr, functionName, args)
}

func (host *vmHost) isDCDTTransferOnReturnDataFromFunctionAndArgs(
	sndAddr, dstAddr []byte,
	functionName string,
	args [][]byte,
) (bool, string, [][]byte) {
	if !host.enableEpochsHandler.IsFlagEnabled(vmhost.MultiDCDTTransferFixOnCallBackFlag) && functionName == core.BuiltInFunctionMultiDCDTNFTTransfer {
		return false, functionName, args
	}

	parsedTransfer, err := host.dcdtTransferParser.ParseDCDTTransfers(sndAddr, dstAddr, functionName, args)
	if err != nil {
		return false, functionName, args
	}
...
func (host *vmHost) determineDestinationForAsyncCall(asyncCallInfo vmhost.AsyncCallInfoHandler) []byte {
	...
	parsedTransfer, err := host.dcdtTransferParser.ParseDCDTTransfers(asyncCallInfo.GetDestination(), asyncCallInfo.GetDestination(), functionName, args)
	if err != nil {
		return asyncCallInfo.GetDestination()
	}
...
func (host *vmHost) determineAsyncCallExecutionMode(asyncCallInfo *vmhost.AsyncCallInfo) (vmhost.AsyncCallExecutionMode, error) {
	...
	if host.IsBuiltinFunctionName(functionName) {
		if sameShard {
			isDCDTTransfer, _, _ := host.isDCDTTransferOnReturnDataFromFunctionAndArgs(runtime.GetContextAddress(), actualDestination, functionName, args)
...
func (host *vmHost) executeSyncCallbackCall(
	...
	if host.enableEpochsHandler.IsFlagEnabled(vmhost.MultiDCDTTransferFixOnCallBackFlag) {
		actualDestination = host.determineDestinationForAsyncCall(asyncCallInfo)
	}
...
func (host *vmHost) createCallbackContractCallInput(
	...
	if destinationErr == nil && destinationVMOutput.ReturnCode == vmcommon.Ok {
		...
		// [0, result1, result2, ....]
		isDCDTOnCallBack, functionName, dcdtArgs = host.isDCDTTransferOnReturnDataWithNoAdditionalData(callbackInitiator, runtime.GetContextAddress(), destinationVMOutput)
	...
	if isDCDTOnCallBack {
		...
		contractCallInput.Arguments = append(contractCallInput.Arguments, dcdtArgs...)
...
func (host *vmHost) isSameShardNFTTransfer(contractCallInput *vmcommon.ContractCallInput) bool {
	...
	return contractCallInput.Function == core.BuiltInFunctionMultiDCDTNFTTransfer ||
		contractCallInput.Function == core.BuiltInFunctionDCDTNFTTransfer
}
