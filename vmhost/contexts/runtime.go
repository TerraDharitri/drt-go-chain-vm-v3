...
func (context *runtimeContext) checkBackwardCompatibility() error {
	...
	if context.iTracker.Instance().IsFunctionImported("getDCDTLocalRoles") {
		return vmhost.ErrContractInvalid
	}
...
func (context *runtimeContext) checkIfContainsNewManagedCryptoAPI() error {
	if context.iTracker.Instance().IsFunctionImported("managedIsDCDTFrozen") {
		return vmhost.ErrContractInvalid
	}
	if context.iTracker.Instance().IsFunctionImported("managedIsDCDTPaused") {
		return vmhost.ErrContractInvalid
	}
	if context.iTracker.Instance().IsFunctionImported("managedIsDCDTLimitedTransfer") {
		return vmhost.ErrContractInvalid
	}
